...
| Runner group | `athena-selfhosted` (id 3), `visibility=selected` restricted to `athena-infra`, `athena-app` and `athena-gitops` (`athena-app`/`athena-gitops` added 2026-08-05 after their Phase 3 self-hosted jobs — media-ci build/publish/handoff, promote.yml resolve — were observed queueing forever against a group that still only granted `athena-infra`), `allows_public_repositories=true` — created live via the runners-groups API, since the org's Default group (id 1) ships `allows_public_repositories=false` and would silently refuse jobs from every repo in this estate (all four are public). Runner-group repo access is API-managed state, NOT Terraform-managed: a job that sits `queued` while runners show online/idle means CHECK THE GROUP'S REPOSITORY ACCESS FIRST (`GET /orgs/AuraBit/actions/runner-groups/3/repositories`) |
